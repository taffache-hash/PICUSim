...
def test_v580_package_facts_are_filesystem_not_paper():
    ...
    assert counts["module_py_non_init_count"] == len(
        [p for p in (ROOT / "modules").rglob("*.py") if p.name != "__init__.py"]
    )
    ...
    # Step 5.9 adds later files; the Step 5.8 facts stay as a frozen preflight snapshot.
